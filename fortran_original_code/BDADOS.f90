...
!***********************************************************************
    ...
    ELSE
        W(NC) = (- DLOG(PC(NC)/14.696D0) - 5.92714D0 + 6.09648D0/Tbr +  &
                 1.28862D0*DLOG(Tbr) - 0.1693478D0*(Tbr**6))/(15.2518 - &
                 15.6875D0/Tbr - 13.4721D0*DLOG(Tbr) +                  &
                 0.43577D0*(Tbr**6))
    END IF
    
    TC(NC) = TC(NC)*0.56D0 !Conversão °R -> K
    PC(NC) = PC(NC)*0.0689476D0 !Conversão psia -> bar
    !-------------------------------------------------------------------
...
304  FORMAT(   20X, f7.3)
305  FORMAT(4/,21X, F6.4) ! Novo formato para não pegar os Z(H2S)=0
401  FORMAT(3/,29X, f6.4)
402  FORMAT(   29X, f6.4)       
403  FORMAT(4/,28X, f7.3)
404  FORMAT(   28X, f7.3)
405  FORMAT(4/,29X, f6.4) ! Novo formato para não pegar Z(H2S) =0
501  FORMAT(3/,37X, f6.4)
502  FORMAT(   37X, f6.4)       
503  FORMAT(4/,36X, f7.3)
504  FORMAT(   36X, f7.3)
505  FORMAT(4/,37X, f6.4) ! Novo formato para não pegar Z(H2S) =0
...
704  FORMAT(   52X, f7.3)
705  FORMAT(4/,53X, f6.4) ! Novo formato para não pegar Z(H2S) =0
801  FORMAT(3/,61X, f6.4)
802  FORMAT(   61X, f6.4)       
803  FORMAT(4/,60X, f7.3)
804  FORMAT(   60X, f7.3)
805  FORMAT(4/,61X, f6.4) ! Novo formato para não pegar Z(H2S) =0
901  FORMAT(3/,69X, f6.4)
902  FORMAT(   69X, f6.4)       
903  FORMAT(4/,68X, f7.3)
904  FORMAT(   68X, f7.3)
905  FORMAT(4/,69X, f6.4) ! Novo formato para não pegar Z(H2S) =0
1001 FORMAT(3/,77X, f6.4)
1002 FORMAT(   77X, f6.4)       
1003 FORMAT(4/,76X, f7.3)
1004 FORMAT(   76X, f7.3)
1005 FORMAT(4/,77X, f6.4) ! Novo formato para não pegar Z(H2S) =0
1101 FORMAT(3/,85X, f6.4)
1102 FORMAT(   85X, f6.4)       
1103 FORMAT(4/,84X, f7.3)
1104 FORMAT(   84X, f7.3)
1105 FORMAT(4/,85X, f6.4) ! Novo formato para não pegar Z(H2S) =0
